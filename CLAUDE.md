...
## Barra de módulos del dashboard (`DashboardStats.js`)

Fila de botones/dropdowns fija arriba de todo el dashboard admin (`sticky top-0`). Desktop: `hidden md:flex flex-wrap` (no `flex-nowrap`) — en pantallas más chicas que la que se usó para armarla originalmente, los botones que no entran pasan a una segunda línea en vez de cortarse. **No usar `overflow-x-auto` para esto**: los dropdowns (`StatCardDropdown`) y tooltips (`StatCard` con prop `tooltip`) cuelgan `position: absolute` del botón: si el contenedor de la fila tiene `overflow-x` no-visible, el navegador fuerza `overflow-y` a recortar también (no se puede tener `overflow-x:auto` + `overflow-y:visible` en el mismo elemento), lo que corta esos menús en **cualquier** pantalla, no solo la chica.

- **`StatCardDropdown` prop `iconOnly`**: oculta el label de texto y el `min-w-[115px]` fijo, dejando solo el ícono (con el label como `title` nativo). Usado en Bestellungen (mismo patrón visual que Cuenta/Redaktion/Analytics/FAQ, que ya eran solo-ícono vía `StatCard`).
- **`StatCardDropdown` items con `{ heading: "..." }`**: agrupa ítems dentro de un mismo dropdown con un separador chiquito no-clickeable, sin agregar un submenú/flyout (más interacción de la que amerita un puñado de ítems). Usado en "Inhalte", que fusiona Artikel/Ausgaben con Aktuelles/Veranstaltungen (antes eran dos dropdowns separados; se fusionaron para ahorrar un botón entero en la fila).
- **`StatCard` prop `tooltip`** (ya existía, no se usaba): acepta JSX, se renderiza en un popup oscuro con flechita (mismo estilo que DIGIabo/GLOBila/VACAPila). Se usó para diferenciar Redaktion (`FaClipboardList`, estadísticas de producción editorial) de Analytics (`FaChartLine`, tráfico del sitio) — antes ambos usaban íconos de gráfico casi idénticos (`FaChartBar` vs `FaChartLine`) y eran indistinguibles a simple vista.
- El menú mobile (`md:hidden`, JSX aparte más abajo en el mismo archivo) es una lista plana con `<Link>` y encabezados de sección — mantenerlo en el mismo orden que la barra desktop cuando se reordene algo ahí (p. ej. Bestellungen vive después de Verwaltung en ambos).
...
- **Portadas anterior/siguiente que se asoman** (rotadas ±5°, `float-left`/`-right`, 120px de ancho): sin `shadow-lg` fijo en la `<Image>` — solo queda el `hover:shadow-2xl` del contenedor (al pasar el mouse). Un `shadow-lg` permanente ahí se veía como una sombra difusa flotando sola en el margen vacío entre la portada y la grilla. Ese margen (`lg:mr-4` en `leftColRef`, antes `lg:mr-1`) es lo que le da aire a esas portadas asomadas para que no queden pegadas a las cards de artículos.

#### Columna derecha: stats, grid y nube de tags (solo desktop `lg`)

- **Barra de stats arriba del todo**: reemplaza el viejo título "Beiträge in dieser Ausgabe". Un IIFE calcula nº de artículos, autores, regiones y traducidos ES del dossier, y los muestra en una barra (`hidden lg:flex … border …`) con un link "Alle ansehen"/"Ver todos" al final separado por `border-l`.
- **Nube de tags con layout tipo revista (CSS float)**: la nube de regiones/temas/autores arranca **debajo del grid** de artículos y, al llegar a la altura donde termina la card flotada de la izquierda (banners), **fluye por debajo de ella** a todo el ancho. Esto exige que el contenedor de fila sea `lg:block` (no flex — los hijos flotados se ignoran dentro de un flex) y que la columna derecha sea `lg:w-auto` (un BFC al lado de un float se encoge al ancho restante; con `w-full` se iría abajo del float). Hay un **clearfix** (`<div className="hidden lg:block lg:clear-both">`) para contener el float aunque haya pocos tags.
- **Grid de artículos a 3 columnas desde 1400px** (`md:grid-cols-2 min-[1400px]:grid-cols-3`, antes `2xl`/1536px): en anchos intermedios (p. ej. MacBook 13", ~1430px lógicos) la grilla a 2 columnas quedaba más alta que la portada+banners de la izquierda (float) y dejaba un **hueco en blanco a la izquierda** por debajo de donde terminaba la portada — un bloque que evita superponerse a un float (grid, flex, `overflow:hidden`…) calcula su ancho **una sola vez para toda su altura**, no se reacomoda cuando el float ya terminó más arriba.
  - **Dos intentos fallidos acá — no repetir**: (1) Pasar portada+resultados a un CSS Grid de dos columnas independientes: un grid reserva el ancho de cada columna para *toda la fila*, no lo libera cuando una columna termina antes que la otra — el hueco quedó **permanente** en vez de desaparecer (y de paso rompió el flow-wrap de la nube de tags, que dependía del float). (2) Cards de artículos como floats reales (no CSS grid) con ancho en `%`: el `width:%` de un float se calcula sobre el ancho **total** del contenedor, ignorando al float vecino — solo la *posición* se reacomoda por fila, no el ancho — así que las cards salieron gigantes (mitad de la página entera, no del espacio libre al lado de la portada).
  - La solución que quedó es simplemente adelantar el breakpoint de 3 columnas (más cards por fila → grid más bajo → en la mayoría de los dossiers ya no le gana en altura a la izquierda). Barato, sin tocar el mecanismo de float. `1400px` (no `xl`/1280px) a propósito: a 1280px, con la portada restando ~450px, cada card de 3 columnas quedaría en ~255px — más angosta que cualquier card del sitio.
