"use client";

import { useLocale } from "next-intl";

export default function ParticipatePage() {
  const locale = useLocale();

  const contentDe = (
    <article className="prose prose-lg max-w-3xl mx-auto py-10 prose-headings:text-red-700 prose-a:text-red-600">
      <h1>Mitmachen</h1>

      <p>
        Habt ihr Lust, bei uns mitzumachen? Dann seid ihr jederzeit herzlich
        willkommen. Ob als Praktikant*in, Social-Media-Expert*in, kreativer Kopf
        in der Redaktion, Hintergrund-Hero der Website oder einfach mal
        Schnuppern bei unseren monatlichen Redaktionssitzungen, schreibt uns
        gerne und kommt vorbei. Mitmachen geht von überall auf der Welt. Die ila
        bietet Raum für kreative Entfaltung, kritischen Austausch und aktives
        Engagement. Als Organisation mit wenig Geld sind wir auf solidarische
        Menschen angewiesen. Euer Engagement kann zum Beispiel so aussehen:
      </p>

      <h2>Analyst*in und kreative Feder</h2>
      <p>
        Du hast Lust auf journalistisches Arbeiten, Interesse, dich kritisch mit
        politischen und gesellschaftlichen Entwicklungen in Lateinamerika
        auseinanderzusetzen? Du möchtest dem europäischen Verhältnis zu dem
        Subkontinent auf den Grund gehen? Du willst Menschen interviewen, die in
        sozialen Bewegungen aktiv sind oder nerdiges Nischenwissen haben? Dann
        schreib für uns! Wir sind immer auf der Suche nach guten Artikeln und
        Interviews für unser monatliches Schwerpunktdossier und die Sektionen
        Aktuelles und Kultur in der Zeitschrift.
      </p>

      <h2>Allroundwunder-Praktikant*in</h2>
      <p>
        Als Praktikant*in kannst du überall einmal reinschauen. Wir bieten
        sowohl freiwillige als auch Pflichtpraktika für 6 Wochen bis 3 Monate
        an. Du kannst aktiv unsere Ausgaben mitgestalten, Veranstaltungen
        mitorganisieren, dein Social-Media-Laster ausleben, politische
        Diskussionen führen und mit uns Strategien überlegen, wie sich die ila
        weiterentwickeln kann und wie wir eine solidarische Zukunft gestalten.
        Journalistische Erfahrungen und/oder Sprachkenntnisse sind schön, aber
        nicht unbedingt notwendig. Da wir fast ausschließlich ehrenamtlich
        arbeiten, können wir leider für die Praktikumszeit keine Vergütung
        zahlen. Natürlich stellen wir am Ende des Praktikums ein Zeugnis aus.
        Hast du Lust? Dann schick uns eine formlose Mail, aus dem hervorgeht,
        wer du bist, wann und warum du bei uns ein Praktikum machen möchtest.
        Wir melden uns dann.
      </p>

      <blockquote>
        „Ich habe eine Leidenschaft entdeckt. Es war ein tolles Erlebnis zu
        merken, wie gut ich in einer Arbeit bin, für die ich mich interessiere.
        Mitgeholfen habe ich zum Beispiel bei der Redaktion und dem Layout. Ich
        habe viel gelesen und viel gelernt. Britt und Mia haben mir viel
        zugetraut und mir Raum gegeben, um meine Ideen umzusetzen. Es war kein
        hierarchisches Arbeitsumfeld, dadurch habe ich mich wohlgefühlt und
        konnte mich öffnen. Ich fühlte mich ernstgenommen. Faszinierend fand
        ich, wie viele Menschen hinter der ila stecken, ohne einen Profit davon
        zu haben. Wir machen das echt aus dem Herzen, weil wir es wichtig
        finden, dass es unabhängigen und linken Journalismus gibt.“
        <footer>— Ximena Juárez Villalpando, ehemalige ila-Praktikantin</footer>
      </blockquote>

      <h2>Herz der Redaktion</h2>
      <p>
        Hast du Lust bekommen, längerfristig bei uns mitzumachen? Dann kannst du
        gerne an einer Redaktionssitzung teilnehmen und uns kennenlernen. Die
        ist am ersten oder zweiten Dienstag im Monat um 19 Uhr. Schreib uns eine
        Mail, wir teilen dir dann den Termin des nächsten Redaktionstreffens
        mit. Teilnehmen geht vor Ort in Bonn oder virtuell von überall aus.
      </p>

      <h2>Hintergrund-Hero Web</h2>
      <p>
        Eine schicke neue Website pflegt sich nicht von selbst – sondern von
        fleißigen Heros im Hintergrund. Sie pflegen auf Zuruf und aus
        Eigeninitiative aktuelle Infos, Online-Artikel und
        Veranstaltungshinweise auf unsere Homepage ein. Sei Teil des kleinen
        heldenhaften Teams!
      </p>

      <h2>Sprachgenie</h2>
      <p>
        Uns erreichen qualitativ hochwertige Artikel auf verschiedenen Sprachen,
        vor allem Spanisch, aber auch Portugiesisch, Englisch, Französisch und
        Niederländisch. Die müssen ins Deutsche übersetzt werden. Außerdem
        werden viele ila-Artikel ins Spanische übersetzt. Du hast Spaß daran,
        die Texte für unsere deutsch- und spanischsprachige Leser*innenschaft
        aufzubereiten? Genial, denn Verstärkung für unser Übersetzungsteam
        brauchen wir immer! Das funktioniert so:
      </p>
      <p>
        <strong>Übersetzungen ins Deutsche:</strong> Über eine
        Übersetzer*innenmailingliste kündigen wir zu übersetzende Texte an. Wer
        Zeit und Lust hat, meldet sich über die Liste. Manchmal hat man zwei
        Wochen zum Übersetzen Zeit, manchmal nur ein/zwei Tage – je nachdem, wie
        nah wir am Redaktionsschluss sind.
      </p>
      <p>
        <strong>Übersetzungen ins Spanische:</strong> Werden von unserer
        Redaktion „web en español“ teilautomatisiert angefertigt und
        anschließend korrigiert. Immer ab dem letzten Samstag im Monat bis zum
        darauffolgenden Mittwoch.
      </p>

      <h2>Marketing-Expert*in</h2>
      <p>
        Die ila hat kein Werbeetat. Daher sind wir bei der Werbung neuer
        Abonnent*innen auf unsere Leser*innen angewiesen – also auf euch. Wir
        freuen uns, wenn ihr die ila weitergebt und die Zeitschrift so bekannter
        wird. Noch besser: Ihr legt unsere Flyer überall dort aus, wo Leute
        verkehren, die sich für die ila interessieren könnten: Weltläden,
        Kirchengemeinden, Kulturzentren, Bildungseinrichtungen, Kneipen usw.
        Gerade zu Semesterbeginn bieten sich besonders die Unis an. Überlegt
        doch mal, ob ihr bei euren täglichen Gängen so etwas ohne großen Aufwand
        machen könnt. Werbematerialien und/oder kostenlose Probehefte zum
        Weitergeben schicken wir euch gerne zu. Anruf oder E-Mail genügt!
        Wiederverkäufer*innen erhalten den üblichen Buchladenrabatt.
      </p>

      <p className="mt-6 font-semibold">
        Interesse? Schreib uns:{" "}
        <a href="mailto:ila-bonn@t-online.de">ila-bonn@t-online.de</a>
      </p>
    </article>
  );

  const contentEs = (
    <article className="prose prose-lg max-w-3xl mx-auto py-10 prose-headings:text-red-700 prose-a:text-red-600">
      <h1>Participar</h1>

      <p>
        ¿Quieres unirte a nosotras y nosotros? Entonces eres siempre bienvenid@.
        Ya sea como practicante, experta en redes sociales, cabeza creativa en
        la redacción, heroína del backstage de la web o simplemente curioseando
        en nuestras reuniones mensuales, escríbenos y acércate. Participar se
        puede desde cualquier lugar del mundo. la ila ofrece espacio para la
        creatividad, el intercambio crítico y el compromiso activo. Como
        organización con pocos recursos dependemos de personas solidarias. Tu
        participación puede tomar muchas formas:
      </p>

      <h2>Analista y pluma creativa</h2>
      <p>
        ¿Te interesa el trabajo periodístico? ¿Quieres analizar críticamente los
        procesos políticos y sociales en América Latina? ¿Quieres entrevistar a
        personas activas en movimientos sociales o con saberes especializados?
        ¡Entonces escribe para nosotras! Siempre buscamos buenos artículos e
        entrevistas para nuestro dossier mensual y las secciones de Actualidad y
        Cultura.
      </p>

      <h2>Practicant@ todoterreno</h2>
      <p>
        Como practicante puedes ver de todo un poco. Ofrecemos tanto prácticas
        voluntarias como obligatorias de 6 semanas a 3 meses. Puedes participar
        en la producción de nuestras ediciones, coorganizar eventos, gestionar
        redes sociales, debatir sobre política y pensar con nosotras estrategias
        para el futuro de la ila. No es imprescindible tener experiencia
        periodística o idiomas. Como casi todo nuestro trabajo es voluntario, no
        podemos pagar las prácticas, pero sí entregamos un certificado. Si te
        interesa, mándanos un correo contándonos quién eres, cuándo y por qué
        quieres hacer tu práctica.
      </p>

      <blockquote>
        “Descubrí una pasión. Fue una experiencia increíble darme cuenta de lo
        bien que me iba en un trabajo que me interesaba. Ayudé, por ejemplo, en
        la redacción y el diseño. Leí mucho y aprendí un montón. Britt y Mia
        confiaron en mí y me dieron espacio para llevar a cabo mis ideas. No fue
        un entorno jerárquico, lo cual me hizo sentir cómoda y abierta. Me sentí
        tomada en serio. Me impresionó la cantidad de personas detrás de la ila,
        sin obtener beneficio alguno. Lo hacemos de corazón, porque creemos que
        es importante que exista un periodismo independiente y de izquierda.”
        <footer>— Ximena Juárez Villalpando, ex practicante de la ila</footer>
      </blockquote>

      <h2>Corazón de la redacción</h2>
      <p>
        ¿Quieres comprometerte a largo plazo? Ven a una reunión de redacción y
        conócenos. Son el primer o segundo martes del mes a las 19h. Escríbenos
        un correo y te mandamos la fecha exacta. Puedes participar en Bonn o de
        forma virtual desde cualquier lugar.
      </p>

      <h2>Heroín@ del backstage web</h2>
      <p>
        Una web bonita no se mantiene sola: necesitamos héroes y heroínas
        discretos que la alimenten con información, artículos y eventos. ¡Únete
        al pequeño equipo detrás de la pantalla!
      </p>

      <h2>Geni@ de los idiomas</h2>
      <p>
        Recibimos artículos de gran calidad en español, portugués, inglés,
        francés y neerlandés que deben traducirse al alemán. Y muchos artículos
        de la ila se traducen al español. ¿Quieres ayudar a preparar textos para
        nuestro público germanohablante e hispanohablante? ¡Perfecto! Siempre
        necesitamos apoyo en el equipo de traducción. Así funciona:
      </p>
      <p>
        <strong>Traducciones al alemán:</strong> Tenemos una lista de correo
        para traductoras y traductores. Anunciamos allí los textos y quien
        quiera se apunta. A veces hay dos semanas para traducir, a veces solo
        uno o dos días.
      </p>
      <p>
        <strong>Traducciones al español:</strong> Se hacen desde nuestra
        redacción web en español de manera semiautomatizada y luego corregidas.
        Siempre del último sábado del mes al miércoles siguiente.
      </p>

      <h2>Expert@ en marketing</h2>
      <p>
        La ila no tiene presupuesto publicitario. Dependemos de nuestra
        comunidad para conseguir nuevas suscripciones. Puedes difundir la ila,
        repartir volantes o ejemplares de muestra en lugares como librerías,
        parroquias, centros culturales, universidades o bares. Al inicio del
        semestre, las universidades son ideales. Si quieres materiales, te los
        enviamos. También ofrecemos descuento a revendedores.
      </p>

      <p className="mt-6 font-semibold">
        ¿Te interesa? Escríbenos:{" "}
        <a href="mailto:ila-bonn@t-online.de">ila-bonn@t-online.de</a>
      </p>
    </article>
  );

  return <>{locale === "es" ? contentEs : contentDe}</>;
}
